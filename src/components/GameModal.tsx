...
import { 
  ...
  Heart, Link as LinkIcon, Check // Импорт новых иконок
} from 'lucide-react';
...
const GameModal: React.FC<GameModalProps> = ({ 
  ...
  const [copySuccess, setCopySuccess] = useState(false); // Стейт для уведомления

  const ICON_SIZE = 16;
  // const ICON_STROKE = 2.5; // (Оставлено закомментированным, если не используется, чтобы TS не ругался)
  ...
  useEffect(() => {
    ...
    setCopySuccess(false); // Сброс при открытии новой игры
  }, [game.id]);
